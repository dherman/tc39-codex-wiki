...
function push(message, cb) {
    ...
    async.waterfall([
        // git clone, git checkout gh-pages
        cloneGHPages,
...
// function isdir(name, cb) {
//     path.exists(name, function(exists) {
//         if (!exists) return cb(null, true);
//         fs.stat(name, function(err, status) {
//             if (err) return cb(err);
//             cb(null, status.isDirectory());
//         });
//     });
// }

function cloneGHPages(cb) {
    var ghpages = git(ghpagesDir);
    path.exists(path.join(ghpagesDir, ".git"), function(exists) {
        if (exists) return cb();
        ghpages.clone(config.repo, function(err) {
            if (err) return cb(err);
