...
function BuildQueue(ghpagesDir, baseURL, sourceURL) {
...
BuildQueue.prototype.request = function request(name, url) {
    ...
    this.building = true;
    var self = this;
    buildAndPush(name, url, this.ghpagesDir, this.baseURL, this.sourceURL, function() {
        self.building = false;
...
function buildAndPush(name, url, ghpagesDir, baseURL, sourceURL, cb) {
    ...
    async.series([
        ...
        // push to gh-pages
        function(cb) {
            push(ghpagesDir, message, cb);
        }
    ], cb);
}

function webhook(port, github, ghpagesDir, baseURL, sourceURL) {
    ...
    var queue = new BuildQueue(ghpagesDir, baseURL, sourceURL);
    ...
    app.post('/', function(req, res) {
        ...
        if (github.indexOf(req.connection.remoteAddress) === -1) {
            goaway(res);
            return;
        }
