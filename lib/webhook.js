...
function BuildQueue(baseURL, sourceURL) {
...
BuildQueue.prototype.request = function request(name, url) {
    ...
    this.building = true;
    var self = this;
    buildAndPush(name, url, this.baseURL, this.sourceURL, function() {
        self.building = false;
...
function buildAndPush(name, url, baseURL, sourceURL, cb) {
...
function webhook(port, githubWebhookIPAddresses, baseURL, sourceURL) {
    ...
    var queue = new BuildQueue(baseURL, sourceURL);
