...
function BuildQueue(sourceURL, cname) {
...
BuildQueue.prototype.request = function request(name, url) {
    ...
    this.building = true;
    var self = this;
    buildAndPush(name, url, this.sourceURL, this.cname, function() {
        self.building = false;
...
function buildAndPush(name, url, sourceURL, cname, cb) {
...
function webhook(port, githubWebhookIPAddresses, sourceURL, cname) {
    ...
    var queue = new BuildQueue(sourceURL, cname);
